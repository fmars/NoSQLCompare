...
int main(){
	...
	try {
		...
		ThriftGen::Schema schema;
		  ...
		  if(client->table_exists(ns,"tt_d"))
			  client->table_drop(ns,"tt_d",false);

			  client->table_create(ns, "tt_d", schema);
/*
		  HqlResult result,r;
...
*/
		...
		cout<<"Total cells: "<<cells.size()<<endl;
		  ThriftGen::Mutator mutator = client->mutator_open(ns, "tt_d", 0, 0);
